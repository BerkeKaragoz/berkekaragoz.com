...
1. `locale`: It stores the locale values of the different parts of the system.
2. `locale -a`: Installs the language packages which already exist on the system.
3. `sudo locale-gen`: Installs the packages inside **`/etc/locale.gen`** which aren't already installed.

#### Files

1. `/etc/default/locale`: the file which references to the **`locale`** command. It stores the values of the locales categorized by POSIX and GNU C library. When needed, according to these values, it installs the locale packages if they aren't already installed. The **`LANG`** value, determines the main locale of the system.
...
2. `/etc/locale.gen`: It has the values of the needed locale packages to be installed. When the command **`sudo locale-gen`** is called, it read the values that aren't commented (`#`) out and installs them. By default, it takes the locale list from `/usr/share/i18n/SUPPORTED` and holds the ones which are not installed as commented.
